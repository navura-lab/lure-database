// scripts/scrapers/longin.ts
// Longin (longin.jp) product page scraper
// Fetch-only — no Playwright or cheerio needed.
//
// Site: Static HTML
// Product URL pattern: https://www.longin.jp/products_{slug}.html
// Spec tables, color swatches, YouTube links
// Types: ミノー, バイブレーション, ペンシル, メタルジグ
// Target fish: シーバス, ヒラメ, 青物
...
const MANUFACTURER = 'Longin';
const MANUFACTURER_SLUG = 'longin';
const SITE_BASE = 'https://www.longin.jp';
const DEFAULT_TARGET_FISH = ['シーバス', 'ヒラメ', '青物'];
...
function log(msg: string): void {
  console.log(`[${timestamp()}] [longin] ${msg}`);
}
...
function deriveTargetFish(name: string, description: string): string[] {
  ...
  if (/青物|ショアジギ|ブリ|ハマチ/i.test(combined)) fish.push('青物');
  if (/メバル|メバリング/i.test(combined)) fish.push('メバル');
  if (/チヌ|クロダイ|チニング/i.test(combined)) fish.push('チヌ');
  if (/マダイ|タイラバ|鯛/i.test(combined)) fish.push('マダイ');
...
export const scrapeLonginPage: ScraperFunction = async (url: string): Promise<ScrapedLure> => {
  ...
  if (!name) {
    ...
    if (titleMatch) name = stripHtml(titleMatch[1]).replace(/\s*[|｜].*$/, '').replace(/\s*-\s*Longin.*$/i, '').trim();
  }
  if (!name) {
  ...
  try {
    ...
    // products_{slug}.html -> slug
    const match = filename.match(/products?_(.+)\.html?$/i);
    if (match) {
      slug = match[1].toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/^-+|-+$/g, '');
    } else {
      slug = filename.replace(/\.html?$/i, '').toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/^-+|-+$/g, '');
    }
  } catch { /* ignore */ }
  ...
  for (const dl of dlMatches) {
    const dtMatches = dl.match(/<dt[^>]*>([\s\S]*?)<\/dt>\s*<dd[^>]*>([\s\S]*?)<\/dd>/gi) || [];
  ...
  if (price === 0) {
    const priceMatch = bodyText.match(/(?:定価|価格|PRICE)[：:\s]*([\d,]+)\s*円/i);
  ...
  // Pattern 1: <figure> with img + figcaption
  ...
  // Pattern 2: color images identified by src
  if (colors.length === 0) {
  ...
  // Pattern 3: gallery list items
  if (colors.length === 0) {
    const galleryMatches = html.match(/<(?:li|div)[^>]*class=["'][^"']*(?:gallery|color|thumb)[^"']*["'][^>]*>[\s\S]*?<\/(?:li|div)>/gi) || [];
