...
function log(msg: string): void {
  console.log(`[${timestamp()}] [pozidrive-garage] ${msg}`);
}
...
function deriveTargetFish(name: string, description: string): string[] {
  ...
  if (/青物|ブリ|ヒラマサ/i.test(combined)) fish.push('青物');
...
export const scrapePozidriveGaragePage: ScraperFunction = async (url: string): Promise<ScrapedLure> => {
