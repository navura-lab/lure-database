// scripts/scrapers/skagit.ts
// Skagit Designs (skagitwebshop.com / skagit-designs.com) product page scraper
// Fetch-only — no Playwright or cheerio needed.
//
// Site: WordPress / e-commerce shop
// Product URL pattern: varies (shop pages, WordPress posts)
// Types: ミノー, シンキングペンシル, ポッパー
// Target fish: シーバス, ヒラメ
...
const MANUFACTURER = 'Skagit Designs';
const MANUFACTURER_SLUG = 'skagit';
const SITE_BASE = 'https://skagitwebshop.com';
const DEFAULT_TARGET_FISH = ['シーバス', 'ヒラメ'];
...
function log(msg: string): void {
  console.log(`[${timestamp()}] [skagit] ${msg}`);
}
...
function deriveTargetFish(name: string, description: string): string[] {
  ...
  if (/青物|ショアジギ|ブリ/i.test(combined)) fish.push('青物');
...
export const scrapeSkagitPage: ScraperFunction = async (url: string): Promise<ScrapedLure> => {
  ...
  // Determine site base from URL for resolving relative links
  ...
  if (!name) {
    const h2Match = html.match(/<h2[^>]*class=["'][^"']*product[^"']*["'][^>]*>([\s\S]*?)<\/h2>/i)
      || html.match(/<h2[^>]*>([\s\S]*?)<\/h2>/i);
    if (h2Match) name = stripHtml(h2Match[1]).trim();
  }
  if (!name) {
    const titleMatch = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
    if (titleMatch) name = stripHtml(titleMatch[1]).replace(/\s*[|｜–—].*$/, '').replace(/\s*Skagit.*$/i, '').trim();
  }
  if (!name) {
  ...
  if (!mainImage) {
    const imgMatch = html.match(/<img[^>]+src=["']([^"']+(?:product|main|hero)[^"']*)["']/i);
  ...
  if (!description) {
    // Try entry-content or product-description div
    const contentMatch = html.match(/<div[^>]*class=["'][^"']*(?:entry-content|product-description|product_description)[^"']*["'][^>]*>([\s\S]*?)<\/div>/i);
    if (contentMatch) {
  ...
  if (!description) {
    ...
    for (const p of pMatches) {
      const text = stripHtml(p).trim();
      if (text.length > 30 && !/spec|スペック|カラー|color|価格|price|重量|copyright/i.test(text.substring(0, 30))) {
        description = text.substring(0, 500);
        break;
      }
    }
  ...
  // Also parse definition lists
  const dlMatches = html.match(/<dl[\s\S]*?<\/dl>/gi) || [];
  for (const dl of dlMatches) {
    const dtddMatches = dl.match(/<dt[^>]*>([\s\S]*?)<\/dt>\s*<dd[^>]*>([\s\S]*?)<\/dd>/gi) || [];
    for (const dtdd of dtddMatches) {
      const dtMatch = dtdd.match(/<dt[^>]*>([\s\S]*?)<\/dt>/i);
      const ddMatch = dtdd.match(/<dd[^>]*>([\s\S]*?)<\/dd>/i);
      if (dtMatch && ddMatch) {
        const label = stripHtml(dtMatch[1]).toLowerCase();
  ...
  // Pattern 2: color images with alt text
  if (colors.length === 0) {
