...
function log(msg: string): void {
  console.log(`[${timestamp()}] [pickup] ${msg}`);
}
...
function deriveTargetFish(name: string, description: string): string[] {
  ...
  if (/青物|ショアジギ|ブリ|ハマチ/i.test(combined)) fish.push('ブリ');
  if (/ヒラマサ/i.test(combined)) fish.push('ヒラマサ');
  if (/カンパチ/i.test(combined)) fish.push('カンパチ');
  if (/マグロ/i.test(combined)) fish.push('マグロ');
...
export const scrapePickupPage: ScraperFunction = async (url: string): Promise<ScrapedLure> => {
