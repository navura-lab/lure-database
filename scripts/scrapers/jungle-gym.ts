...
function log(msg: string): void {
  console.log(`[${timestamp()}] [jungle-gym] ${msg}`);
}
...
function deriveTargetFish(name: string, description: string): string[] {
  ...
  if (/カサゴ|根魚|ロック/i.test(combined)) fish.push('ロックフィッシュ');
...
export const scrapeJungleGymPage: ScraperFunction = async (url: string): Promise<ScrapedLure> => {
  ...
  // Pattern 2: figures with figcaption
  if (colors.length === 0) {
