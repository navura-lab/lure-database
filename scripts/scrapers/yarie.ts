// scripts/scrapers/yarie.ts
// Yarie (yarie.co.jp / etanba.co.jp) product page scraper
// Fetch-only — no Playwright or cheerio needed.
//
// Site: Static HTML (etanba.co.jp is the actual host)
// Products organized by number (No.677, No.731, etc.)
// Types: スプーン, クランク, ミノー (area trout focused)
// Target fish: トラウト
...
const MANUFACTURER = 'Yarie';
const MANUFACTURER_SLUG = 'yarie';
const SITE_BASE = 'https://www.etanba.co.jp';
...
const TYPE_KEYWORDS: [RegExp, string][] = [
  ...
  [/ワーム|worm/i, 'ワーム'],
  [/フック|hook|フォーセップ|forceps/i, 'フック'],
  [/アクセサリー|accessory/i, 'アクセサリー'],
];
...
function log(msg: string): void {
  console.log(`[${timestamp()}] [yarie] ${msg}`);
}
...
function deriveTargetFish(name: string, description: string): string[] {
  ...
  if (/トラウト|trout|マス|鱒|エリア/i.test(combined)) fish.push('トラウト');
...
export const scrapeYariePage: ScraperFunction = async (url: string): Promise<ScrapedLure> => {
  ...
  if (!mainImage) {
    const imgMatch = html.match(/<img[^>]+src=["']([^"']+(?:product|image|lure)[^"']*\.(?:jpg|jpeg|png|webp|gif))["']/i);
  ...
  if (!description) {
    ...
    for (const p of pMatches) {
      ...
      if (text.length > 30 && !/spec|スペック|カラー|copyright|navigation/i.test(text.substring(0, 30))) {
  ...
  // Yarie often lists specs inline (e.g. "50mm total length, Sinking 3.6g, ¥550")
