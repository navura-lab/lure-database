// scripts/_test-gamakatsu.ts
// Quick test for the Gamakatsu scraper — run against 4 diverse product pages.
// Usage: cd lure-database && npx tsx scripts/_test-gamakatsu.ts

import { scrapeGamakatsuPage } from './scrapers/gamakatsu.js';

var TEST_URLS = [
  // Hard bait - Crankbait (AVENGE CRANK 400, 8 colors with gallery)
  'https://www.gamakatsu.co.jp/products/80-604/',
  // Worm - Lightgame (宵姫ノレソレ 1.8inch, small worm)
  'https://www.gamakatsu.co.jp/products/19241/',
  // Egi (エヴォリッジ ベーシック 3.0号, multi-size)
  'https://www.gamakatsu.co.jp/products/19221/',
  // Octopus lure (オクトライズ ウキウキクロー)
  'https://www.gamakatsu.co.jp/products/19-417/',
];

async function main() {
  console.log('=== Gamakatsu Scraper Test ===\n');
  var allPassed = true;
  ...
  for (var i = 0; i < TEST_URLS.length; i++) {
    ...
    try {
      var result = await scrapeGamakatsuPage(url);
