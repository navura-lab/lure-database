// scripts/_test-madness.ts
// Quick smoke-test for the MADNESS scraper.
// Usage: npx tsx scripts/_test-madness.ts

import { scrapeMadnessPage } from './scrapers/madness.js';

const TEST_URLS = [
  'https://www.madness.co.jp/products/salt/shiriten-vibe73',  // バイブレーション (multiple chart sections)
  'https://www.madness.co.jp/products/worm/separ9',           // ワーム (2 size variants, LOT field)
  'https://www.madness.co.jp/products/bass/balam-300',         // ビッグベイト (bass)
];
...
async function main() {
  ...
  for (var i = 0; i < TEST_URLS.length; i++) {
    ...
    try {
      var result = await scrapeMadnessPage(url);
      ...
      if (result.manufacturer !== 'MADNESS') errors.push('manufacturer is not MADNESS');
