// _test-geecrack.ts — Quick validation for the GEECRACK scraper
// Run: npx tsx scripts/_test-geecrack.ts

import { scrapeGeecrackPage } from './scrapers/geecrack.js';

var tests = [
  {
    label: 'CHORO UO (Soft Bait / Bass)',
    url: 'https://www.geecrack.com/bass/product/detail/?id=241',
    expectSlug: 'choro-uo',
    expectType: 'ワーム',
    expectColorsMin: 5,
    expectPrice: true,
  ...
  {
    label: 'SUPAKU SWIMMER (Hard Bait / Bass)',
    url: 'https://www.geecrack.com/bass/product/detail/?id=5',
    expectSlug: 'supaku-swimmer',
    expectType: 'ルアー',
    expectColorsMin: 3,
    expectPrice: true,
  },
  {
    label: 'Saltwater product (Ika)',
    url: 'https://www.geecrack.com/saltwater/product/detail/?id=133',
    expectSlug: '',  // will just check non-empty
    expectType: '',  // won't check
    expectColorsMin: 1,
    expectPrice: true,
  },
];
...
async function runTests() {
  ...
  for (var i = 0; i < tests.length; i++) {
    ...
    try {
      var result = await scrapeGeecrackPage(t.url);
      var errors: string[] = [];

      if (t.expectSlug && result.slug !== t.expectSlug) {
        errors.push('slug: got "' + result.slug + '", expected "' + t.expectSlug + '"');
      }
      if (!result.slug) {
        errors.push('slug: empty');
      }
      if (t.expectType && result.type !== t.expectType) {
